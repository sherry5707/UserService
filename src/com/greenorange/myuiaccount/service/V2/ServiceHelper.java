...
public class ServiceHelper {
    ...
    public static String getToken(String method, String version, String param, String timestamp) throws Exception {
        return SecureUtil.md5(Config.getChannel() + method + version + param + timestamp + Config.getChannelkey());
    }
    //add by sherry for ip
    public static String getToken(String method, String version,String timestamp) throws Exception {
        return SecureUtil.md5(Config.getChannel() + method + version+ timestamp + Config.getChannelkey());
    }
    ...
    public static String getServiceURL(String method, String param) throws Exception {
        ...
        try {
            ...
            url.appendQueryParameter("version", version);
            url.appendQueryParameter("params", encodeparam);
            url.appendQueryParameter("timestamp", timestamp);
            url.appendQueryParameter("token", getToken(method, version,	encodeparam, timestamp));
    ...
    public static RequestParams getServiceRequestParams(String method, BaseRequestParam param)throws Exception{
    	try{
    		if(param.checkValid()){
    			return getServiceRequestParams(method, param.getRequestParam());
    		}
    	}catch(Throwable e){}
    ...
    private static RequestParams getServiceRequestParams(String method, String param)throws Exception{
        if(TextUtils.isEmpty(method))throw new Exception("invalid method");
        if(TextUtils.isEmpty(param))throw new Exception("invalid param");
        String encodeparam = param;
        try{
    ...
    private static RequestParams getIPRequestParams(String method)throws Exception{
        ...
        try{
            encodeparam = getEncodeParam(encodeparam);
            android.util.Log.e("ServiceHelper", "encodeparam-->"+encodeparam);
        }catch (Throwable e){e.printStackTrace();throw new Exception("invalid param");}
        try {
            String timestamp = String.valueOf(System.currentTimeMillis());
            String version = Config.getVersion(method);
            ...
            requestParams.put("timestamp", timestamp);
            requestParams.put("token", getToken(method, version,encodeparam,timestamp));
            android.util.Log.e("getIPParams","requestParams:"+requestParams.toString());
            return requestParams;
        }catch (Throwable e){
            e.printStackTrace();
            android.util.Log.e("getIPParams","exception:"+e.getMessage());
            throw new Exception("unkonw exception");
        }
    }
